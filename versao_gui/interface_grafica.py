...
class App(ctk.CTk):
    ...
    def executar_analise_command(self):
        ...
        try:
            intervalo_selecionado = self.combo_intervalos.get()
            print(f"Intervalo selecionado: '{intervalo_selecionado}'")  # DEBUG
            
            intervalo_limpo = intervalo_selecionado.strip('[] ').replace(' ', '')
            print(f"Intervalo limpo: '{intervalo_limpo}'")  # DEBUG
            
            a, b = intervalo_limpo.split(',')
            ...
            print(f"Comando completo: {comando}")  # DEBUG
            
            resultado = subprocess.run(comando, capture_output=True, text=True, check=True, encoding='utf-8',errors='ignore')
